...
export default function KontrakanSlider({ kosList }: { kosList: Kos[] }) {
  const sliderRef = useRef<HTMLDivElement>(null);

  const scrollSlider = (direction: 'left' | 'right') => {
    if (sliderRef.current) {
      const isRtl = typeof window !== 'undefined' && window.getComputedStyle(sliderRef.current).direction === 'rtl';
      let scrollAmount = 344;
      if (isRtl) {
        // In RTL, to go visually right (Next for LTR user), we need to scroll left (negative).
        // So direction 'right' (Next) scrolls left (-344), and 'left' (Back) scrolls right (+344).
        scrollAmount = direction === 'left' ? scrollAmount : -scrollAmount;
      } else {
        scrollAmount = direction === 'left' ? -scrollAmount : scrollAmount;
      }
  ...
  const kontrakanList = kosList.filter(kos => kos.kos_type?.toLowerCase() === 'kontrakan');
  ...
  return (
    <div className="flex flex-col lg:flex-row-reverse gap-8 lg:gap-12 items-start w-[calc(100%+50px)] -mr-[50px] lg:-mr-0 lg:-ml-[50px]">
      ...
      <div className="w-full lg:flex-1 min-w-0">
        <div
          ref={sliderRef}
          className="flex gap-6 overflow-x-auto snap-x snap-mandatory pb-8 pt-2 pr-[50px] lg:pr-0 lg:pl-[50px] lg:[direction:rtl] [&::-webkit-scrollbar]:hidden"
          style={{ msOverflowStyle: 'none', scrollbarWidth: 'none' }}
        >
          {kontrakanList.length > 0 ? (
            kontrakanList.map((kos) => (
              <Link key={kos.id} href={`/detail/${kos.slug}`} className="group cursor-pointer block w-[320px] shrink-0 snap-start lg:[direction:ltr]">
            ...
            <div className="w-full py-20 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2">Kontrakan Tidak Ditemukan</h3>
            </div>
          )}
        </div>
      </div>
